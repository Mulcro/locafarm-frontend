...
const UserOrders = ({onClose}) => {
    ...
    const deleteOrder = (order_id,listing_id) => {
        fetch(`https://locafarm-backend-35edbc31d82d.herokuapp.com/orders/${order_id}/delete`,{
            method: "DELETE",
            headers:{
        ...
        .catch(err => {
            console.log(err);
            toast.error("Something went wrong. Your order was not deleted, please try again");
        })
    }
                                    ...
                                    <div className="grid grid-cols-3 gap-5">
                                        {orders.map((order, index) => (
                                            <div key={index} id={order.id} className="md:grid-cols-2 lg:grid-cols-3 gap-6 p-4 bg-white dark:bg-gray-800 rounded-lg overflow-hidden shadow-lg transform hover:scale-105 transition duration-300 ease-in-out">
                                                ...
                                                <button onClick={() => deleteOrder(order.id, order.listing_id)} className='mx-auto  text-white bg-indigo-400 border-2 border-black border-solid p-1 rounded-xl text-sm font-bold hover:bg-indigo-500'>Delete Order</button>
                                            </div>
                                                ))
                                            }
                                            <div/>
